...
setInterval(() => {
  if (totalRequestAllUsers !== totalRequestAllUsersWrittenToLogFile) {
    writeLogToGitHub(`Wrote usage data to GitHub since ${process.env.MAXIMUM_MINUTES_BEFORE_WRITING_LOG} minutes went by even though only ${totalRequestAllUsers - totalRequestAllUsersWrittenToLogFile} new requests had been made`);
    ...
    if (totalRequestAllUsers === 1 || totalRequestAllUsers - totalRequestAllUsersWrittenToLogFile >= process.env.NUMBER_OF_REQUESTS_BEFORE_WRITING_LOG) {
      setImmediate((arg) => {
        ...
        if (totalRequestAllUsers === 1) {
          ...
          commitMessage = `Wrote usage data to GitHub since ${totalRequestAllUsers - totalRequestAllUsersWrittenToLogFile} new requests had been made` 
        }
...
function writeLogToGitHub(commitMessage) {
  ...
  PIDUsageGroupedByPID.forEach(usageDataForPID => {
    usageData += `xxx\t${usageDataForPID.PID}\t${usageDataForPID.totalRequestsSum}\t${usageDataForPID.fullName}\r\n`;
  });
