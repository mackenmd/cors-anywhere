...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    log.log(`PIDD:${displayPID} (${PIDUsage[PID]}) - ${originShortName} - ${eventKey}`)
...
function getUsageData(usageID) {
  ...
  for (const [key, value] of Object.entries(PIDUsage)) {
    if (usageID.length > 0) { 
      usageData += `${usageID}\t${key}\t${value}\r\n`;
    } else {
      usageData += `0\t${key}\t${value}\r\n`;
    }
    numberOfRecords += 1;
    numberOfServerRequests += value || 0;
  }
