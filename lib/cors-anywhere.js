...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    if (usageDataForPID) {
      ...
      usageDataForPID = {"PID" : PID, "eventKey" : eventKey, "totalRequests" : 1};
      PIDUsage.push(usageDataForPID);
    }

    let displayPID = PID === "3477" ? " ME " : PID;
    log.log(`PID:${displayPID} (${usageDataForPID.totalRequests}) - ${originShortName} - ${eventKey}`)
...
function getUsageDataWithEvents(usageID) {
  ...
  PIDUsage.forEach(usageDataForPID => {
    usageData += `${usageID}\t${usageDataForPID.PID}\t${usageDataForPID.eventKey}\t${usageDataForPID.totalRequests}\r\n`;
