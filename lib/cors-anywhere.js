...
let eventKeyRegEx = /(?:\/)(\d{4,})(?:\/)/

let PIDUsage = [];
let logFilesha;
let logURL;

let totalRequestAllUsersWrittenToLogFile = 0;
let totalRequestAllUsers = 0;
...
let gitHubtimer = new timer.Timer(() => {
  if (totalRequestAllUsers !== totalRequestAllUsersWrittenToLogFile) {
    writeLogToGitHub(`Writing usage data to GitHub since ${process.env.MAXIMUM_MINUTES_BEFORE_WRITING_LOG} minutes went by.  ${totalRequestAllUsers - totalRequestAllUsersWrittenToLogFile} new requests have been made`);

    totalRequestAllUsersWrittenToLogFile = totalRequestAllUsers;
  }
}, process.env.MAXIMUM_MINUTES_BEFORE_WRITING_LOG * 60 * 1000);
...
function proxyRequest(req, res, proxy) {
  // log.log(`proxyRequest`, log.begin);
  ...
  // log.log(`location.href: ${location.href}`);

  // log.log(`getProxyForUrl: ${proxyThroughUrl}`);



  if (proxyThroughUrl) {
    log.log(`getProxyForUrl is truthy`);
  ...
  try {
    // log.log(`proxy.web`, log.begin);

    proxy.web(req, res, proxyOptions);

    // log.log(`proxy.web`, log.end);

  } catch (err) {
    proxy.emit('error', err, req, res);
  }

  // log.log(`proxyRequest`, log.end);
}
...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    // log.log(`Processing request`, log.begin);

    let commandComponents = req.url.split('888888');

    if(commandComponents.length > 1) {
      let commandData=commandComponents[1];
      if (commandData.length > 0) {
        let commandDataComponents = commandData.toLowerCase().split('=');
        let commandRequest = commandDataComponents[0];

        let commandRequestParameter = "";
        if (commandDataComponents.length > 1) {
          commandRequestParameter = commandDataComponents[1];
        }

        if (commandRequest === "usage") {
          let usageData = getUsageData(commandRequestParameter);
          res.writeHead(200, {'Content-Type': 'text/plain'});
          res.end(usageData);
          return;
        } else if (commandRequest === "usagenames") {
          let usageData = getUsageDataWithNames(commandRequestParameter);
          res.writeHead(200, {'Content-Type': 'text/plain'});
          res.end(usageData);
          return;
        } else if (commandRequest === "usagewithevents") {
          let usageDataWithEvents = getUsageDataWithEvents(commandRequestParameter);
          res.writeHead(200, {'Content-Type': 'text/plain'});
          res.end(usageDataWithEvents);
          return;
        } else if (commandRequest === "usagefortournies") {
          let usageDataWithEvents = getUsageDataForTournies(commandRequestParameter);
          res.writeHead(200, {'Content-Type': 'text/plain'});
          res.end(usageDataWithEvents);
          return;
        } else if (commandRequest === "cleardata") {
          PIDUsage = [];
          log.log("Usage data cleared as requested");
          res.writeHead(200, {'Content-Type': 'text/plain'});
          res.end("Usage data cleared as requested");
          return;
        } else {
          res.writeHead(200, {'Content-Type': 'text/plain'});
          res.end("Invalid command requested");
          return;
        }
      }
    }
    ...
    let PID="";
    let baseReqURL = req.url.split('999999');

    if(baseReqURL.length > 1) {
      req.url = baseReqURL[0];
      PID=baseReqURL[1];
    } else {
      PID="9999999";
    }
    ...
    if (origin.includes("codepen")) {
      originShortName = "DEV ";
    } else if (origin.includes("dufferpools")) {
      originShortName = "prod";
    } else {
      originShortName = origin;
    }
    
    let eventKey = "";
    let regExArray = location.href.match(eventKeyRegEx);
    if (regExArray) {
      eventKey = regExArray[1] || 00000;
    } else {
      eventKey = location.href;
    }

    let usageDataForPID = PIDUsage.find(x => x.PID === PID && x.eventKey === eventKey);

    let fullName = "Person not found";

    if (usageDataForPID) {
      usageDataForPID.totalRequests += 1
      fullName = usageDataForPID.fullName;
    } else {
      let person = people.people.find(x => x.PID == PID);
      if (person) {
        fullName = person.fullName;
      }
      usageDataForPID = {"PID" : PID, "fullName" : fullName,  "eventKey" : eventKey, "totalRequests" : 1};
      PIDUsage.push(usageDataForPID);
    }

    let displayPID = PID === "3477" ? " ME " : PID;
    log.log(`PID:${displayPID} (${usageDataForPID.totalRequests}) - ${originShortName} - ${eventKey} - ${fullName}`)

    
    totalRequestAllUsers += 1;

    if (totalRequestAllUsers === 1 || totalRequestAllUsers - totalRequestAllUsersWrittenToLogFile >= process.env.NUMBER_OF_REQUESTS_BEFORE_WRITING_LOG) {
      setImmediate((arg) => {
        let commitMessage;

        if (totalRequestAllUsers === 1) {
          commitMessage = "First user request has been made so time to create GitHub log file."
        } else {
          commitMessage = `Writing usage data to GitHub since ${totalRequestAllUsers - totalRequestAllUsersWrittenToLogFile} new requests have been made.` 
        }
        writeLogToGitHub(commitMessage);

        totalRequestAllUsersWrittenToLogFile = totalRequestAllUsers;
      }, "Arg parameter is not used at this time");
    ...
    // log.log(`Processing request`, log.end);
  };
}
...
function getUsageDataWithEvents(usageID) {
  let usageData = "";
  let numberOfRecords = 0;
  let numberOfServerRequests = 0;
  let uniqueTournaments = [];
  
  PIDUsage.forEach(usageDataForPID => {
    usageData += `${usageID}\t${usageDataForPID.PID}\t${usageDataForPID.eventKey}\t${usageDataForPID.totalRequests}\t${usageDataForPID.fullName}\r\n`;

    numberOfRecords += 1;
    numberOfServerRequests += usageDataForPID.totalRequests;

    let tourney = uniqueTournaments.find(x => x.eventKey === usageDataForPID.eventKey);
    if (tourney) {
      tourney.totalRequests += 1;
    } else {
      uniqueTournaments.push({"eventKey" : usageDataForPID.eventKey, "totalRequests" : 1});
    }
  });

  if (numberOfRecords === 0) {
    usageData = `there is no usage data  in series ${usageID} since the last time it was cleared`;
    log.log(`usage data requested but ${usageData}`);
  } else {
    log.log(`usage data requested.  There were ${numberOfRecords} different users, ${uniqueTournaments.length} different tournaments, and ${numberOfServerRequests} unique server requests in series ${usageID}`);
  }
...
function getUsageData(usageID) {
  let usageData = "";
  let numberOfUsers = 0;
  let numberOfServerRequests = 0;

  let PIDUsageGroupedByPID = sql.sum3(PIDUsage.sort(order("PID", "eventKey")),
                                      ["PID"],
                                      ["totalRequests"],
                                      "CNT");

  PIDUsageGroupedByPID.forEach(usageDataForPID => {
    numberOfServerRequests += usageDataForPID.totalRequestsSum;
    numberOfUsers += 1;

    usageData += `${usageID}\t${usageDataForPID.PID}\t${usageDataForPID.totalRequestsSum}\r\n`;
  });
  ...
  if (numberOfUsers === 0) {
    ...
    log.log(`usage data requested.  There were ${numberOfUsers} different users and ${numberOfServerRequests} unique server requests in series ${usageID}`);
  }
...
function getUsageDataWithNames(usageID) {
  let usageData = "";
  let numberOfUsers = 0;
  let numberOfServerRequests = 0;
  ...
  PIDUsageGroupedByPID.forEach(usageDataForPID => {
    ...
    usageData += `${usageID}\t${usageDataForPID.PID}\t${usageDataForPID.totalRequestsSum}\t${usageDataForPID.fullName}\r\n`;
  });

  if (numberOfUsers === 0) {
    usageData = `there is no usage data  in series ${usageID} since the last time it was cleared`;
    log.log(`usage data with names requested but ${usageData}`);
  } else {
    log.log(`usage data with names.  There were ${numberOfUsers} different users and ${numberOfServerRequests} unique server requests in series ${usageID}`);
  }
...
function getUsageDataForTournies(usageID) {
  let usageData = "";
  let numberOfTournies = 0;
  let numberOfServerRequests = 0;

  let PIDUsageGroupedByPID = sql.sum3(PIDUsage.sort(order("eventKey")),
                                      ["eventKey"],
                                      ["totalRequests"],
                                      "CNT");

  PIDUsageGroupedByPID.forEach(usageDataForTourney => {
    numberOfServerRequests += usageDataForTourney.totalRequestsSum;
    numberOfTournies += 1;

    usageData += `${usageID}\t${usageDataForTourney.eventKey}\t${usageDataForTourney.totalRequestsSum}\r\n`;
  });

  if (numberOfTournies === 0) {
    usageData = `there is no usage data in series ${usageID} since the last time it was cleared`;
    log.log(`usage data requested but ${usageData}`);
  } else {
    log.log(`usage data requested.  There were ${numberOfTournies} different tournaments and ${numberOfServerRequests} unique server requests in series ${usageID}`);
  }
...
function writeLogToGitHub(commitMessage) {
  if (!logURL) { 
    const now  =  new Date();
    const dateString = date.format(now,'YYYY-MM-DD---HH-mm-ss');

    logURL = `https://api.github.com/repos/${process.env.GITHUB_LOG_REPO_OWNER}/${process.env.GITHUB_LOG_REPO_NAME}/contents/DPUsage-${dateString}`;
  }

  let auth = `Bearer ${process.env.GITHUB_LOG_REPO_TOKEN}`;
  ...
  PIDUsageGroupedByPID.forEach(usageDataForPID => {
    usageData += `${process.env.CURRENT_USAGE_SERIES}\t${usageDataForPID.PID}\t${usageDataForPID.totalRequestsSum}\t${usageDataForPID.fullName}\r\n`;
  });

  let buff = Buffer.from(usageData).toString('base64');
  ...
  log.log(`---------  ${commitMessage}  ---------`);

  let responseInJSON = fetch(logURL,
    {
      method: "PUT",
      headers: {
  ...
  .then(file => {
    ...
    logFilesha = file.content.sha
  })
  .catch((error) => {
    log.log(`---------  Unable to write to GitHub usage file - error: ${error}  ---------`);
  });;
